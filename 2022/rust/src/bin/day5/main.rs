...
fn do_work(
    lines: &Vec<String>,
    instr_func: &dyn Fn(&mut Vec<Vec<String>>, usize, usize, usize) -> (),
) -> String {
    ...
    instrs
        ...
        .map(|v| {
            (
                v[1].parse::<usize>().expect("Unable to parse"),
                v[3].parse::<usize>().expect("Unable to parse"),
                v[5].parse::<usize>().expect("Unable to parse"),
            )
        })
        .for_each(|(n, src, dst)| instr_func(&mut stacks, n, src, dst));
    stacks.iter().fold("".to_string(), |acc, stack| {
        acc + &stack[stack.len() - 1]
            .to_string()
            .replace("[", "")
            .replace("]", "")
    })
...
fn part1(lines: &Vec<String>) -> String {
    do_work(lines, &|stacks: &mut Vec<Vec<String>>,
                     n: usize,
                     src: usize,
                     dst: usize| {
        for _ in 0..n {
...
fn part2(lines: &Vec<String>) -> String {
    do_work(lines, &|stacks: &mut Vec<Vec<String>>,
        ...
        stacks.clone()[src - 1]
            ...
            .take(n)
            .rev()
            .for_each(|c| stacks[dst - 1].push(c.to_string()));
        for _ in 0..n {
            stacks[src - 1].pop();
        }
    })
