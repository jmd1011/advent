...
object Day3 extends App {
    ...
    def calcNum(schematic: Schematic, coord: Coord): Int = {
        schematic(coord) match {
            case Some(c) if c.isDigit => c.asDigit + 10 * calcNum(schematic, Coord(coord.x - 1, coord.y))
            case _ => 0
        }
    }

    def doWork(lines: List[List[Char]])(skip: Char => Boolean)(update: (Schematic, (Int, List[Coord]), List[Coord]) => (Int, List[Coord])): Int = {
        val schematic = new Schematic(lines)
        val coords = lines.zipWithIndex.map {
            case (l, i) => l.zipWithIndex.map {
                case (c, j) => (c, Coord(j, i))
            }
        }.flatten
        coords.foldLeft((0, List[Coord]())) {
            case (cur, (c, _)) if skip(c) => cur
            case (cur, (_, coord)) => {
                update(schematic, cur, coord.neighbors.filter(n => schematic(n) match {
    ...
    println(part1(Source.fromFile("2023/input/day03/real").getLines().toList.map(_.toList)))
    println(part2(Source.fromFile("2023/input/day03/real").getLines().toList.map(_.toList)))
}
