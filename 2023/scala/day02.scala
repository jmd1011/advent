...
object Day2 extends App {
  ...
  def play(turns: String)(calcPoints: Score => Int): Int = {
    calcPoints(
      turns
        ...
        .foldLeft(start)((m, e) =>
          new Score(
            math.max(m.red, e.red),
            math.max(m.green, e.green),
            math.max(m.blue, e.blue)
          )
        )
    )
  }
  ...
  def part1(lines: Iterator[String]): Int = doWork(lines)(id =>
    x => if (x.red <= 12 && x.green <= 13 && x.blue <= 14) id else 0
  )
  def part2(lines: Iterator[String]): Int =
    doWork(lines)(id => x => x.red * x.green * x.blue)
